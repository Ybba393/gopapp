...
// ── Sub-components (same as exit-ticket) ────────────────────────────────────
...
function MultiChoice({ options, value, onChange, disabled, maxSelect }: {
  ...
  return (
    <View style={styles.optionList}>
      {maxSelect ? <Text style={styles.optionHint}>Select up to {maxSelect}</Text> : null}
      {options.map((opt) => {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      if (form) {
        ...
        const qs: Question[] = (form.questions ?? []).map((q: any, i: number) =>
          typeof q === 'string'
    ...
    return (
      <View style={[styles.root, { paddingTop: insets.top }]}>
        ...
        <View style={styles.doneWrap}>
          ...
          <Text style={styles.doneSub}>This form hasn't been set up yet.</Text>
          <TouchableOpacity style={styles.doneBtn} onPress={() => router.back()}>
            <Text style={styles.doneBtnText}>Back</Text>
          </TouchableOpacity>
        </View>
  ...
  return (
    ...
    >
      ...
      <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
        <Text style={styles.qCount}>{questions.length} questions</Text>

        {questions.map((q, index) => (
          <View key={q.id} style={styles.questionCard}>
            <Text style={styles.qNumber}>{index + 1}.</Text>
            <Text style={styles.qText}>{q.text}</Text>
            {q.required === false && <Text style={styles.optionalTag}>Optional</Text>}
...
const styles = StyleSheet.create({
  ...
  headerTitle: { color: '#fff', fontSize: 20, fontWeight: '800', lineHeight: 26 },
  scroll: { padding: 16, paddingBottom: 48, gap: 12 },
  qCount: { fontSize: 12, color: colors.textMuted, fontWeight: '600', marginBottom: 4 },
  questionCard: {
  ...
  qText: { fontSize: 15, fontWeight: '700', color: colors.textPrimary, lineHeight: 21, marginBottom: 10 },
  optionalTag: { fontSize: 11, color: colors.textMuted, fontStyle: 'italic', marginBottom: 10, marginTop: -6 },
  ...
  optionRow: {
    ...
    padding: 12, borderRadius: 12, borderWidth: 1.5, borderColor: '#E8EAED',
    backgroundColor: '#FAFAFA',
  },
  optionRowSelected: { borderColor: colors.primary, backgroundColor: '#EEF3FA' },
  optionLabel: { fontSize: 14, color: colors.textPrimary, flex: 1, fontWeight: '500' },
