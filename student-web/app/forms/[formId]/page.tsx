...
export default function FormPage() {
  ...
  useEffect(() => {
    ...
    supabase.auth.getUser().then(async ({ data: { user } }) => {
      ...
      setUserId(user.id)
      const [{ data: f }, { data: existing }] = await Promise.all([
        supabase.from('standalone_forms').select('*').eq('id', formId).single(),
        supabase.from('form_responses').select('id').eq('respondent_id', user.id).eq('form_id', formId).single(),
      ])
      if (f) {
        setForm(f)
        const qs = (f.questions ?? []).map((q: any, i: number) =>
  ...
  if (alreadyDone || submitted) return (
    <div className="min-h-screen flex flex-col items-center justify-center px-6 text-center" style={{ background: '#0D2137' }}>
      ...
      <h1 className="text-2xl font-black text-white mb-2">Submitted!</h1>
      <p className="text-white/60 text-sm mb-8">Thank you for filling out {form?.title}.</p>
  ...
  return (
    <div style={{ background: '#F5F7FA', minHeight: '100vh' }}>
      <div className="px-5 pt-14 pb-6" style={{ background: '#0D2137' }}>
        <button onClick={() => router.push('/roadmap')} className="text-white/60 text-sm mb-3">← Back</button>
        <p className="text-xs font-bold tracking-widest mb-1" style={{ color: '#D4A853' }}>FORM</p>
        <h1 className="text-xl font-black text-white">{form?.title}</h1>
        {form?.description && <p className="text-white/60 text-sm mt-1">{form.description}</p>}
      </div>
